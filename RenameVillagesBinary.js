		...
		$('.quickedit-vn').each(function(key,village){ //each village
			...
			for(var j=0; j<config.length; j++) //each configuration option
			{
				if(config[j].enabled)
				{
					...
					{
						var number = dec2bin(key+Number(config[j].startNum));
					...
					{
						var namePool = ['Cloud','Software','Domain','VPN','VPS','IP','Exploit','Breach','Firewall','Malware','Virus','Ransomware','Trojan Horse','Worm','Botnet','Bot','Spyware','Rootkit','DDoS','Phishing','Spear Phishing','Encryption','BYOD','Pentest','S.Engineering','Clickjacking','Deepfake','White Hat','Grey Hat','Black Hat','Red Team','Blue Team'];
						for(var k=0; k<3; k++)
						{
							let tmp = btoa("A VERY LONG STRING");
							while (tmp.length > 16){
								tmp = btoa(namePool[Math.round(Math.random()*(namePool.length-1))])
								tmp = crypto.randomUUID().trim().toString();
							}
							name += tmp;
						}
					}
