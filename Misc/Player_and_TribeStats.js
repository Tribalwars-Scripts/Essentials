...
// @version			1.2
// @include https://***.tribalwars.**/game.php?*screen=info_player*
// @include https://***.tribalwars.**/game.php?*screen=info_ally*
// @include https://**.tribalwars.**/game.php?*mode=profile&screen=ally*
...
function main() {
	...
	const server = game_data.market;
	const srv = game_data.world;
	const graphType = game_data.screen.split('_')[1];
	try {
		var id = location.href.match(/[?&]id=(\d+)/)[1];
	} catch (e) {
		var id = game_data.player.ally_id;
	}
	const isPlayer = !!game_data.screen.match('info_player');
